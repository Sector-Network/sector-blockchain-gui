...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const sectorper_input = ip_parsed.limit;
    ...
    const sectorper_value = Number.parseInt(Number(sectorper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (sector per interval):{' '}
                  {octet_to_sector_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>

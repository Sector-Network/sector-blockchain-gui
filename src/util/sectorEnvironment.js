...
const getSectorVersion = () => {
  let version = null;
  const exePath = getExecutablePath('sector');
  // first see if we can get a sector exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or sector is on the path
    try {
      version = child_process

...
global.daemon_rpc_ws = `wss://${self_hostname}:56800`;
...
function loadConfig(net) {
  try {
    // check if SECTOR_ROOT is set. it overrides 'net'
    const config_root_dir =
      'SECTOR_ROOT' in process.env
        ? process.env.SECTOR_ROOT
        : path.join(os.homedir(), '.sector', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 56800); // jshint ignore:line

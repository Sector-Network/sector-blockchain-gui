...
import sectorEnvironment from '../util/sectorEnvironment';
import sectorConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!sectorEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    sectorConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!sectorConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !sectorConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (sectorConfig.manageDaemonLifetime()) {
        sectorEnvironment.startSectorDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Sector Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Sector-Network/sector-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Sector-Network/sector-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Sector-Network/sector-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Sector-Network/sector-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Sector-Network/sector-blockchain/issues',
              );
            },
          },
          /*{
    ...
    if (process.platform === 'darwin') {
      // Sector Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Sector' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Sector Blockchain' }),
            click: () => {
              openAbout();
            },
          },
